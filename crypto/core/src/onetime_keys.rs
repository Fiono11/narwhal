...
use crate::{domain_separators::HASH_TO_SCALAR_DOMAIN_TAG, ristretto::{RistrettoPublic, RistrettoPrivate}, account_keys::PublicAddress};
...
pub fn create_tx_out_target_key(
    ...
    let Hs: Scalar = {
        ...
        let C: &RistrettoPoint = &RistrettoPoint::from(*view_public.as_ref());
        hash_to_scalar(r * C)
    };

    let spend_public = recipient.spend_public_key();
    let D: &RistrettoPoint = &RistrettoPoint::from(*spend_public.as_ref());
